...
/// Determine which nodes are the culmination of a Part, Chapter,
/// Section, etc. This is computed in a similar way to h-index.
pub fn compute_heading_levels_from_tree<T>(
	sorted_nodes: &Vec<Rc<RefCell<Node<T>>>>
) -> [usize; 5] {
	let mut heading_counts: [usize; 5] = [0; 5];

	// Collect number of predecessors from nodes and rank from fewest to
	// most predecessors
	let rev_ranked_branch_counts: Vec<usize> = {
		let mut rrbc: Vec<usize> = vec![0; sorted_nodes.len()];
		for n in sorted_nodes {
			let p = n.borrow().num_predecessors_fast();
			rrbc[p] += 1;
		}
		rrbc.sort_unstable();
		rrbc
	...
	// Compute number of predecessors necessary to generate a given
	// heading level; start with the highest heading level
	let mut h = 2;
	heading_counts[0] = {
		let mut hc = 0;
		'whileloop: while h < rev_ranked_branch_counts.len() {
			if rev_ranked_branch_counts[h] > 1 {
				h = h + 1;
				hc = rev_ranked_branch_counts[h - 1];
				break 'whileloop;
			}
		}
		hc
	...
	// Compute number of predecessors necessary to generate a given
	// heading level; continue with lower heading levels
	let mut heading_depth: usize = 1;
	while heading_depth < 6 && h < rev_ranked_branch_counts.len() {
		if rev_ranked_branch_counts[h] > 1 {
			if heading_counts[h - 1] <= rev_ranked_branch_counts[h] {
				heading_counts[heading_depth] = rev_ranked_branch_counts[h];
				heading_depth += 1;
			}
		}
		h += 1;
